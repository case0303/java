...
public class ContructorExample {
	...
	// - public class는 파일 이름이기 때문에 그 소스 파일 전체의 주체를 알수 있는
	//   이름을 짓는 것이 좋다.
	...
	class Car{
		...
		//생성자는 새로운 인스턴스를 생성 할 때 가장 먼저 호출되기 때문에
		//값을 초기화하는 용도로 많이 사용된다.
		
		Car(String wheel, int numOfWheel, String brand, String name){
			//this ; 현재 인스턴스를 의미함
			
			//클래스 변수와 매개변수를 같은 이름으로 사용할 때
			//클래스 변수를 지칭하기 위해서 "this"를 붙혀야 한다.
			this.wheel = wheel;
